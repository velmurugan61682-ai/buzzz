...
const makePool = (rows = [{}]) => {
  ...
  return {
    log,
    query: async (text, params) => { log.push({ text: String(text).replace(/\s+/g, " ").trim(), params }); return { rows }; },
  };
};
...
for (const [name, fn] of scoped) {
  ...
  const scopedQuery = /workspace_id = \$1/.test(q.text)
...
for (const [name, fn] of [
  ...
  try { await fn(); ok(false, `${name} should refuse a missing workspace`); }
  catch (e) { ok(e.code === "missing_scope", `${name} refuses a query with no tenant scope`); }
}
...
try { await failDb.createWorkspace({ name: "Acme", ownerId: "u1" }); ok(false, "a failed step should throw"); }
catch { ok(failPool.log.includes("ROLLBACK"), "a half created workspace is rolled back, never left behind"); }

/* ---- metrics: measured or null, never a filled in zero ---- */
const mPool = makePool([{ workspaces: 3, activeUsers7d: 11, signups7d: 2, messagesToday: 40,
  aiActions24h: 9, approvalsPending: 1, workflowFailures24h: 0, integrationFailures24h: 0, apiErrors24h: 0 }]);
...
ok(api.updateAudit === undefined && api.deleteAudit === undefined,
...
/* ---- update guard ---- */
try { await db.updateContact("ws1", "c1", {}); ok(false, "an empty patch should be refused"); }
catch (e) { ok(e.code === "empty_patch", "an empty update is refused rather than issuing a broken query"); }

console.log(fails ? `data layer: ${fails} FAILED` : "data layer: all 30 checks passed");
