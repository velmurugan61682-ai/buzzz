...
import re, sys
src = open(__import__('os').path.join(__import__('os').path.dirname(__file__),'..','client','src','App.jsx')).read()
...
import re as _re
_src = open("client/src/App.jsx", encoding="utf-8").read()
...
_html = open("client/index.html", encoding="utf-8").read()
if "cdn.tailwindcss.com" in _html:
    print("FAIL: client/index.html loads Tailwind from the CDN; build it instead")
    raise SystemExit(1)
if "index.css" not in open("client/src/main.jsx", encoding="utf-8").read():
